...
class MiraiMCHandler(private val plugin: PlumBot, private val bot: IBot): BotHandler {
    override fun onGroupMessage(message: String, groupId: Long, userId: Long) {
//        val lock = Object()
//        var message = ""

//        (bot as Onebot).client.action(GetGroupMemberList(eroupId)) { memberList ->
//            synchronized(lock) {
...
//                "text" -> message += jsonObject.get("data").asJsonObject.get("text").asString
//                "image" -> message += "[图片]"
//                "at" -> message += bot.getGroupUserCard(groupId, jsonObject.get("data").asJsonObject.get("qq").asLong)
//            }
//        }

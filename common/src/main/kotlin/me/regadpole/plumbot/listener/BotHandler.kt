...
//        }
        event.jsonMessage.forEach {
            ...
            when (jsonObject.get("type").asString) {
                "text" -> message += jsonObject.get("data").asJsonObject.get("text").asString
                "image" -> message += "[图片]"
                "at" -> message += bot.getGroupUserCard(event.groupId, jsonObject.get("data").asJsonObject.get("qq").asLong)
            }
        }
...
//            # player_list, player_num, max_player
    ...
    fun onUserDecrease(event: GroupMemberDecreaseEvent) {
        DatabaseProvider.getBindByUser(event.userId.toString()).keys.forEach {
...
//                ))
        ...
        DatabaseProvider.getDatabase()!!.removeBind(event.userId)
    }
}
...
class Onebot(private val config: ConfigMaker, override val client: WebsocketBotClient): BotImpl {
    ...
    override fun start(): BotImpl {
        handler = BotHandler(PlumBot.INSTANCE, this)
        client.connect()
        client.registerEvent(OnebotListener(this))

        if (config.getBooleanFromConfig("feature", "load", "enable")) config.getLongListFromConfig("groups").forEach {
            sendMsg(true, it,
                PlumBot.INSTANCE.messages.load
                , config.getBooleanFromConfig("feature", "load", "pic"))
        }
    ...
    override fun shutdown() {
        if (config.getBooleanFromConfig("feature", "load", "enable")) config.getLongListFromConfig("groups").forEach {
            if (config.getBooleanFromConfig("feature", "load", "pic")) sendGroupPicWithText(it, PlumBot.INSTANCE.messages.unload)
            else sendGroupMsg(it, PlumBot.INSTANCE.messages.unload)
        }
        handler = null
...
//            }
    ...
    private fun loadAllCache(){
        config.getLongListFromConfig("groups").forEach { groupId ->
